...
/**
 * This class helping handle both onSwipe and onCLick events
 * To handle onClick you have to set [clickableArea] and [clickListener]
 */
class ClickableMotionLayout : MotionLayout {
            ...
            when (motionEvent.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    ...
                    if (isAClick(startX, endX, startY, endY)) {
                        clickableArea?.let {
                            if (motionEvent.x.toInt() in (it.left..it.right)
                                && motionEvent.y.toInt() in (it.top..it.bottom))
                                doAClick()
                        }
                        return dispatch
                    }
    ...
    private fun isUserMovingFinger(
        startX: Float?,
        endX: Float,
        startY: Float?,
        endY: Float
    ...
    private fun isAClick(startX: Float?, endX: Float, startY: Float?, endY: Float): Boolean {
        return when {
            ...
            isDuringSwipe() && transitionIsNotInFinalPosition() -> false
            else -> isTouchInClickArea(startX, endX, startY, endY)
        }
    }

    private fun transitionIsNotInFinalPosition(): Boolean = this.progress < 1

    private fun isTouchInClickArea(
        startX: Float,
        endX: Float,
        startY: Float,
        endY: Float
